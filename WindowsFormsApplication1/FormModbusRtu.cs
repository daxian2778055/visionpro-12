...
namespace WindowsFormsApplication1
{
    public partial class FormModbusRtu : Form, IPlcOutputWriter, ICommRuntimeHost, ICommLinkContext
    {
        ...
        internal void InitializeForm()
        {
            ...
            address_qishi = CommGridHelper.ReadIniDecimalIntegral(wdini.ReadString(ModbusRtuIniStore.ConnSection(_linkId), "qishi", "0"), 0, 0, ModbusRtuIniStore.MaxAddress, "连接" + _linkId + "/qishi(总起始地址)", Log);
            address_length = CommGridHelper.ReadIniDecimalIntegral(wdini.ReadString(ModbusRtuIniStore.ConnSection(_linkId), "zongchang", "1"), 1, 0, ModbusRtuIniStore.MaxTotalLength, "连接" + _linkId + "/zongchang(总长度)", Log);
            lunxun_time = CommGridHelper.ReadIniDecimalIntegral(wdini.ReadString(ModbusRtuIniStore.ConnSection(_linkId), "lunxun_time", "20"), 20, 0, ModbusRtuIniStore.MaxPollInterval, "连接" + _linkId + "/lunxun_time", Log);
            ...
            geshu = CommGridHelper.ReadIniInt(wdini.ReadString(ModbusRtuIniStore.ConnSection(_linkId), "geshu", "0"), 0, 0, 10, "连接" + _linkId + "/geshu(块个数)", Log);
            int greenSkipped = 0;
            if (geshu > 0)
            {
                for (int i = 0; i < geshu; i++)
                {
                    ...
                    fins_qishi = CommGridHelper.ReadIniDecimalIntegral(wdini.ReadString(ModbusRtuIniStore.BlockSection(_linkId, i + 1), "qishi", "0"), 0, 0, 60000, "连接" + _linkId + "/块" + (i + 1) + "/qishi", Log);   // ★第33轮复审①：上限 60000 = 块起始 numericUpDown5.Maximum；手改更大的值让轮询/网格点击处的 int.Parse(qishi) 每圈抛 OverflowException。★第33轮复审警告①：改 Integral 版（钳位+取整）——只钳不截时范围内的小数（手改 2.5）会原样进 fins_dic，表头双击回填 dataGridView1_CellMouseDoubleClick 里无保护 int.Parse("2.5") 抛 FormatException = 全局崩溃弹窗
                    fins_length = CommGridHelper.ReadIniDecimalIntegral(wdini.ReadString(ModbusRtuIniStore.BlockSection(_linkId, i + 1), "changdu", "0"), 0, 0, 50, "连接" + _linkId + "/块" + (i + 1) + "/changdu", Log);   // ★第33轮复审③：上限 50 = 界面块长度 numericUpDown4.Maximum；手改更大的值不抛异常，只会让下方回绿循环空转（≥2^31 时 int 计数回绕=永久死循环）。★第33轮复审警告①：改 Integral 版（钳位+取整），范围内小数同样截断，否则 int.Parse("2.5") 在表头双击/轮询处抛 FormatException
        ...
        private void t39_TextChanged(object sender, EventArgs e)
        {
            if (chushihua)
            {
                _heartbeatValue = t39.Text;
                wdini.WriteString(ModbusRtuIniStore.CameraSection(_linkId, 13), "xintiao", t39.Text);
            }
        }
        ...
        private void c16_CheckedChanged(object sender, EventArgs e)
        {
            if (chushihua)
            {
                bool on = c16.CheckState == CheckState.Checked;
                _heartbeatEnabled = on;
                wdini.WriteString(ModbusRtuIniStore.CameraSection(_linkId, 13), "xintiaoen", on ? "true" : "false");
            }
        }
