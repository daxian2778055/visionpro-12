...
namespace WindowsFormsApplication1
{
    public partial class FormOmron : Form, IPlcOutputWriter, ICommRuntimeHost, ICommLinkContext
    {
        ...
        internal void InitializeForm()
        {
          ...
          fins_lunxunen=bool.Parse( wdini.ReadString(FinsIniStore.ConnSection(_linkId), "fins_lunxunen", "false"));
          fins_en = bool.Parse(wdini.ReadString(FinsIniStore.ConnSection(_linkId), "fins_en", "false"));
            address_qishi = decimal.Parse(wdini.ReadString(FinsIniStore.ConnSection(_linkId), "qishi", "0"));
            address_length = decimal.Parse(wdini.ReadString(FinsIniStore.ConnSection(_linkId), "zongchang", "1"));
            lunxun_time = decimal.Parse(wdini.ReadString(FinsIniStore.ConnSection(_linkId), "lunxun_time", "20"));
            ...
            if (geshu > 0)
            {
                for (int i = 0; i < geshu; i++)
                {
                    ...
                    fins_qishi = decimal.Parse(wdini.ReadString(FinsIniStore.BlockSection(_linkId, i + 1), "qishi", "0"));
                    fins_length= decimal.Parse(wdini.ReadString(FinsIniStore.BlockSection(_linkId, i + 1), "changdu", "0"));
            ...
            // camera_dic[13] → 心跳（反馈/返回值）
            textBox53.Text = camera_dic[13][1];
            if (camera_dic[13][2] == "true")
       ...
       public volatile bool fins_en = false;
            ...
            try
            {
                ...
                foreach (var pat in camSnapshot)
                {
                    ...
                    try
                    {
                        ...
                        //   （PLC 侧靠超时判 NG 兜底；不再"写失败也清槽"导致结果与完成字一次丢光）。
                        if (writeAllOk)
                            pat.Value[5] = "无";
                        else
                            LogXieRetry("D 槽 " + pat.Value[5] + " → " + fins_temp);
                    }
        ...
        private void textBox53_TextChanged(object sender, EventArgs e)
        {
            if (chushihua)
            {
                camera_dic[13][1] = textBox53.Text;
                wdini.WriteString(FinsIniStore.CameraSection(_linkId, 13), "fanhuizhi", textBox53.Text);
            }
        }
