    ...
    public partial class FormOmron : Form, IPlcOutputWriter, ICommRuntimeHost, ICommLinkContext
    {
        ...
        internal void InitializeForm()
        {
          ...
          fins_en = CommGridHelper.ReadIniBool(wdini.ReadString(FinsIniStore.ConnSection(_linkId), "fins_en", "false"), false, "连接" + _linkId + "/fins_en", Log);
            address_qishi = CommGridHelper.ReadIniDecimalIntegral(wdini.ReadString(FinsIniStore.ConnSection(_linkId), "qishi", "0"), 0, 0, FinsIniStore.MaxAddress, "连接" + _linkId + "/qishi(总起始地址)", Log);
            address_length = CommGridHelper.ReadIniDecimalIntegral(wdini.ReadString(FinsIniStore.ConnSection(_linkId), "zongchang", "1"), 1, 0, FinsIniStore.MaxTotalLength, "连接" + _linkId + "/zongchang(总长度)", Log);
            lunxun_time = CommGridHelper.ReadIniDecimalIntegral(wdini.ReadString(FinsIniStore.ConnSection(_linkId), "lunxun_time", "20"), 20, 0, FinsIniStore.MaxPollInterval, "连接" + _linkId + "/lunxun_time", Log);
            ...
            if (geshu > 0)
            {
                for (int i = 0; i < geshu; i++)
                {
                    ...
                    fins_qishi = CommGridHelper.ReadIniDecimalIntegral(wdini.ReadString(FinsIniStore.BlockSection(_linkId, i + 1), "qishi", "0"), 0, 0, 60000, "连接" + _linkId + "/块" + (i + 1) + "/qishi", Log);   // ★第33轮复审①：上限 60000 = 块起始 numericUpDown5.Maximum；手改更大的值让轮询/网格点击处的 int.Parse(qishi) 每圈抛 OverflowException。★第33轮复审警告①：改 Integral 版（钳位+取整）——只钳不截时范围内的小数（手改 2.5）会原样进 fins_dic，表头双击回填 dataGridView1_CellMouseDoubleClick 里无保护 int.Parse("2.5") 抛 FormatException = 全局崩溃弹窗
                    fins_length= CommGridHelper.ReadIniDecimalIntegral(wdini.ReadString(FinsIniStore.BlockSection(_linkId, i + 1), "changdu", "0"), 0, 0, 50, "连接" + _linkId + "/块" + (i + 1) + "/changdu", Log);   // ★第33轮复审③：上限 50 = 界面块长度 numericUpDown4.Maximum；手改更大的值不抛异常，只会让下方回绿循环空转（≥2^31 时 int 计数回绕=永久死循环）。★第33轮复审警告①：改 Integral 版（钳位+取整），范围内小数同样截断，否则 int.Parse("2.5") 在表头双击/轮询处抛 FormatException
       ...
       public volatile bool fins_en = false;
        ...
        // ★功能修复①（第41轮）：FINS 数据块节后缀与 FinsIniStore.BlockSection 同构（连接1 = "1".."10"，
        //   连接2~4 = "1_fins2".."10_fins4"）。原硬编码 "fins"/"" → 连接2~4 点"清除"实际擦掉的是连接1的
        //   [fins]+节1..10（自己的配置原样保留、重启复活，连接1 被静默清空）。
        private string DataSuffix { get { return _linkId == 1 ? "" : "_fins" + _linkId.ToString(); } }

        private void button5_Click(object sender, EventArgs e)
        {
            if (!CommGridHelper.TryPrepareFullClear(textBox12.Text.Trim(), () => textBox12.Text = ""))
                return;
            CommGridHelper.PerformFullDataClear(wdini, FinsIniStore.ConnSection(_linkId), DataSuffix, ref geshu, fins_dic, _triggerLatch);
                                    ...
                                    foreach (var par in finsSnapshot)
                                    {
                                        ...
                                        for (int j = 0; j < blkChangdu; j++)
                                        {
                                            if (par.Value[4] == "int")
                                            {
                                                ...
                                                // 读取short变量
                                                DemoUtils.ReadResultRender1(ReadLocked(() => omronFinsNet.ReadInt16("D" + (blkQishi + j).ToString())), "D" + (blkQishi + j).ToString(), out fins_temp);
                                                CommGridHelper.SetPollCell(_gridUi, fins_data, xuanzhong_temp, fins_temp);
                                            ...
                                            else if (par.Value[4] == "string")
                                            {
                                                xuanzhong_temp = blkQishi - baseAddr + j;
                                                // 读取字符串
                                                DemoUtils.ReadResultRender1(ReadLocked(() => omronFinsNet.ReadString("D" + (blkQishi + j).ToString(), 1)), "D" + (blkQishi + j).ToString(), out fins_temp);
                                                CommGridHelper.SetPollCell(_gridUi, fins_data, xuanzhong_temp, fins_temp);
                                                if (CommTriggerHelper.IsReadFailureText(fins_temp)) blockReadFailed = true; // ★R11
                                                shuju_temp += GetMiddleValue(fins_temp, " ", "\r");
                                            }
                                            else if (par.Value[4] == "long" && j % 2 == 0)
                                            {
                                                xuanzhong_temp = blkQishi - baseAddr + j;
                                                // 读取字符串
                                                DemoUtils.ReadResultRender1(ReadLocked(() => omronFinsNet.ReadInt32("D" + (blkQishi + j).ToString())), "D" + (blkQishi + j).ToString(), out fins_temp);
                                                CommGridHelper.SetPollCell(_gridUi, fins_data, xuanzhong_temp, fins_temp);
                                                if (CommTriggerHelper.IsReadFailureText(fins_temp)) blockReadFailed = true; // ★R11
                                                shuju_temp += GetMiddleValue(fins_temp, " ", "\r");
                                            }
                                            else if (par.Value[4] == "float" && j % 2 == 0)
                                            {
                                                xuanzhong_temp = blkQishi - baseAddr + j;
                                                // 读取字符串
                                                DemoUtils.ReadResultRender1(ReadLocked(() => omronFinsNet.ReadFloat("D" + (blkQishi + j).ToString())), "D" + (blkQishi + j).ToString(), out fins_temp);
                                                CommGridHelper.SetPollCell(_gridUi, fins_data, xuanzhong_temp, fins_temp);
                                                if (CommTriggerHelper.IsReadFailureText(fins_temp)) blockReadFailed = true; // ★R11
                                                shuju_temp += GetMiddleValue(fins_temp, " ", "\r");
                                            }
