    ...
    public partial class FormModbus : Form, IPlcOutputWriter, ICommRuntimeHost, ICommLinkContext
    {
        ...
        internal void InitializeForm()
        {
           ...
           // comboBox1.SelectedItem = HslCommunication.Core.DataFormat.CDAB;
            ...
            geshu = (int)CommGridHelper.ReadIniDecimal(wdini.ReadString(ModbusTcpIniStore.ConnSection(_linkId), "geshu", "0"), 0, "连接" + _linkId + "/geshu(块个数)", Log);
            ...
            if (geshu > 0)
            {
                for (int i = 0; i < geshu; i++)
                {
                    ...
                    fins_qishi = CommGridHelper.ReadIniDecimal(wdini.ReadString(ModbusTcpIniStore.BlockSection(_linkId, i + 1), "qishi", "0"), 0, "连接" + _linkId + "/块" + (i + 1) + "/qishi", Log);
                    fins_length = CommGridHelper.ReadIniDecimal(wdini.ReadString(ModbusTcpIniStore.BlockSection(_linkId, i + 1), "changdu", "0"), 0, "连接" + _linkId + "/块" + (i + 1) + "/changdu", Log);
