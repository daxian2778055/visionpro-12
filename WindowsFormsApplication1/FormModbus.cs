    ...
    public partial class FormModbus : Form, IPlcOutputWriter, ICommRuntimeHost, ICommLinkContext
    {
        ...
        internal void InitializeForm()
        {
           ...
           // comboBox1.SelectedItem = HslCommunication.Core.DataFormat.CDAB;
            ...
            //   FormatException（后果同上方注释），改走安全读取取默认值 + 记日志。
            ...
            geshu = CommGridHelper.ReadIniInt(wdini.ReadString(ModbusTcpIniStore.ConnSection(_linkId), "geshu", "0"), 0, 0, 10, "连接" + _linkId + "/geshu(块个数)", Log);
            int greenSkipped = 0;
            if (geshu > 0)
            {
            ...
            _heartbeatEnabled = string.Equals(wdini.ReadString(ModbusTcpIniStore.CameraSection(_linkId, 13), "xintiaoen", camera_dic[13][2]), "true", StringComparison.OrdinalIgnoreCase);
            wdini.WriteString(ModbusTcpIniStore.CameraSection(_linkId, 13), "xintiao", _heartbeatValue);
            wdini.WriteString(ModbusTcpIniStore.CameraSection(_linkId, 13), "xintiaoen", _heartbeatEnabled ? "true" : "false");
        ...
        private async void button1_Click( object sender, EventArgs e )
        {
            ...
            try
            {
                ...
                // ★W5 修复（2026-09-23 第23轮）：建链的破坏性动作（Close 旧链+更换 Client+ConnectServer）
                //   不再在后台线程裸跑，改为持 _ioSync 跑同步 Connect——与轮询读写/心跳/自动重连同锁串行。
                OperateResult connect = await System.Threading.Tasks.Task.Run(
                    () => { lock (_ioSync) { return _modbusLink.Connect( fmtParam ); } } );
        ...
        {
            // 读取bool变量
            ManualRead(() => DemoUtils.ReadResultText( () => busTcpClient.ReadCoil( textBox3.Text ), textBox3.Text, textBox4 ));
        }

        private void button4_Click_1( object sender, EventArgs e )
        {
            // 读取离散输入bool变量
            ManualRead(() => DemoUtils.ReadResultText( () => busTcpClient.ReadDiscrete( textBox3.Text ), textBox3.Text, textBox4 ));
        }

        private void button_read_short_Click( object sender, EventArgs e )
        {
            // 读取short变量
            ManualRead(() => DemoUtils.ReadResultText( () => busTcpClient.ReadInt16( textBox3.Text ), textBox3.Text, textBox4 ));
        ...
        {
            // 读取ushort变量
            ManualRead(() => DemoUtils.ReadResultText( () => busTcpClient.ReadUInt16( textBox3.Text ), textBox3.Text, textBox4 ));
        }

        private void button_read_int_Click( object sender, EventArgs e )
        {
            // 读取int变量
            ManualRead(() => DemoUtils.ReadResultText( () => busTcpClient.ReadInt32(  textBox3.Text ), textBox3.Text, textBox4 ));
        }
        private void button_read_uint_Click( object sender, EventArgs e )
        {
            // 读取uint变量
            ManualRead(() => DemoUtils.ReadResultText( () => busTcpClient.ReadUInt32( textBox3.Text ), textBox3.Text, textBox4 ));
        }
        private void button_read_long_Click( object sender, EventArgs e )
        {
            // 读取long变量
            ManualRead(() => DemoUtils.ReadResultText( () => busTcpClient.ReadInt64( textBox3.Text ), textBox3.Text, textBox4 ));
        }

        private void button_read_ulong_Click( object sender, EventArgs e )
        {
            // 读取ulong变量
            ManualRead(() => DemoUtils.ReadResultText( () => busTcpClient.ReadUInt64( textBox3.Text ), textBox3.Text, textBox4 ));
        }

        private void button_read_float_Click( object sender, EventArgs e )
        {
            // 读取float变量
            ManualRead(() => DemoUtils.ReadResultText( () => busTcpClient.ReadFloat( textBox3.Text ), textBox3.Text, textBox4 ));
        }

        private void button_read_double_Click( object sender, EventArgs e )
        {
            // 读取double变量
            ManualRead(() => DemoUtils.ReadResultText( () => busTcpClient.ReadDouble( textBox3.Text ), textBox3.Text, textBox4 ));
        }

        private void button_read_string_Click( object sender, EventArgs e )
        {
            // 读取字符串
            ManualRead(() => DemoUtils.ReadResultText( () => busTcpClient.ReadString( textBox3.Text , ushort.Parse( textBox5.Text ) ), textBox3.Text, textBox4 ));
        }
        ...
        // ★BUG1/BUG3 修复 + ★W2/W5 修复（2026-09-23 第23轮，与 FormOmron 同构）：
        ...
        private void ManualGuarded(Func<string> op, string errorTitle)
        {
            ...
            else
            {
                try
                {
                    ...
                    else if (busTcpClient == null) msg = "尚未连接 PLC，请先连接。";
                    else
                    {
                        try { msg = op(); }
        ...
        {
            // bool写入
            ManualGuarded(() => DemoUtils.WriteResultText( () => busTcpClient.WriteCoil( textBox8.Text, bool.Parse( textBox7.Text ) ), textBox8.Text ), "写入出错");
        }

        private void button22_Click( object sender, EventArgs e )
        {
            // short写入
            ManualGuarded(() => DemoUtils.WriteResultText( () => busTcpClient.Write( textBox8.Text , short.Parse( textBox7.Text ) ), textBox8.Text ), "写入出错");
        }

        private void button21_Click( object sender, EventArgs e )
        {
            // ushort写入
            ManualGuarded(() => DemoUtils.WriteResultText( () => busTcpClient.Write( textBox8.Text , ushort.Parse( textBox7.Text ) ), textBox8.Text ), "写入出错");
        }
        ...
        {
            // int写入
            ManualGuarded(() => DemoUtils.WriteResultText( () => busTcpClient.Write( textBox8.Text , int.Parse( textBox7.Text ) ), textBox8.Text ), "写入出错");
        }

        private void button19_Click( object sender, EventArgs e )
        {
            // uint写入
            ManualGuarded(() => DemoUtils.WriteResultText( () => busTcpClient.Write( textBox8.Text , uint.Parse( textBox7.Text ) ), textBox8.Text ), "写入出错");
        }

        private void button18_Click( object sender, EventArgs e )
        {
            // long写入
            ManualGuarded(() => DemoUtils.WriteResultText( () => busTcpClient.Write( textBox8.Text , long.Parse( textBox7.Text ) ), textBox8.Text ), "写入出错");
        }

        private void button17_Click( object sender, EventArgs e )
        {
            // ulong写入
            ManualGuarded(() => DemoUtils.WriteResultText( () => busTcpClient.Write( textBox8.Text , ulong.Parse( textBox7.Text ) ), textBox8.Text ), "写入出错");
        }

        private void button16_Click( object sender, EventArgs e )
        {
            // float写入
            ManualGuarded(() => DemoUtils.WriteResultText( () => busTcpClient.Write( textBox8.Text , float.Parse( textBox7.Text ) ), textBox8.Text ), "写入出错");
        }

        private void button15_Click( object sender, EventArgs e )
        {
            // double写入
            ManualGuarded(() => DemoUtils.WriteResultText( () => busTcpClient.Write( textBox8.Text , double.Parse( textBox7.Text ) ), textBox8.Text ), "写入出错");
        }
        ...
        {
            // string写入
            ManualGuarded(() => DemoUtils.WriteResultText( () => busTcpClient.Write( textBox8.Text , textBox7.Text ), textBox8.Text ), "写入出错");
        }
        ...
        {
            ManualGuarded(() => DemoUtils.BulkReadResultText( busTcpClient, textBox6, textBox9, textBox10 ), "读取出错");
        }
        ...
        {
            // ★G6：重连中/未连接时启动压力测试=后台线程 NRE/连打正在重建的 socket（未处理线程异常直接杀进程）
            if (_reconnecting != 0 || busTcpClient == null)
            {
        ...
        private void thread_test2( )
        {
            ...
            try
            {
                while (count > 0)
                {
                    // ★G6：与轮询/心跳/重连互斥（否则并发抢 socket 结果全废）；重连开始即退出，不再连打半死链路
                    if (_reconnecting != 0) break;
                    bool okW, okR;
                    lock (_ioSync)
                    {
        ...
        private void thread_end( )
        {
            ...
            {
                // ★G6：测试期间窗体被关闭/释放后裸 Invoke 会在后台线程抛未处理异常直接杀进程
                try
                {
                        ...
                        if (numericUpDown5.Value >= numericUpDown1.Value && numericUpDown1.Value + numericUpDown2.Value >= numericUpDown4.Value + numericUpDown5.Value)
                        {
                            if (textBox16.Text.Length > 0)
                            {
                                if (fins_dic.Count < 10)
                                {
                                    ...
                                    else if (fins_dic.ContainsKey(textBox16.Text))
                                    {
                                        // ★第33轮：重叠检查只看地址段，同名两块（地址不重叠时）会一路走到
                                        //   fins_dic.Add 抛 ArgumentException = 全局"程序崩溃"弹窗。改为明确提示。
                                        MessageBox.Show("已存在同名数据块「" + textBox16.Text + "」，请换一个名称。");
                                    }
                                    else
                                    {
                                        fins_dic.Add(textBox16.Text, new string[] { textBox16.Text, numericUpDown5.Value.ToString(), numericUpDown4.Value.ToString(), comboBox2.Text, (comboBox3.Text ?? "").Trim().ToLowerInvariant() });   // ★N5：格式列归一化（同 ini 加载路径）
                                    ...
                                    foreach (var par in finsSnapshot)
                                    {
                                        ...
                                        for (int j = 0; j < blkChangdu; j++)
                                        {
                                            if (par.Value[4] == "int")
                                            {
                                                ...
                                                // 读取short变量
                                                DemoUtils.ReadResultRender1(ReadLocked(() => busTcpClient.ReadInt16((blkQishi + j).ToString())), (blkQishi + j).ToString(), out fins_temp);
                                                CommGridHelper.SetPollCell(_gridUi, fins_data, xuanzhong_temp, fins_temp);
                                            ...
                                            else if (par.Value[4] == "string")
                                            {
                                                xuanzhong_temp = blkQishi - baseAddr + j;
                                                // 读取字符串
                                                DemoUtils.ReadResultRender1(ReadLocked(() => busTcpClient.ReadString((blkQishi + j).ToString(), 1)), (blkQishi + j).ToString(), out fins_temp);
                                                CommGridHelper.SetPollCell(_gridUi, fins_data, xuanzhong_temp, fins_temp);
                                                if (CommTriggerHelper.IsReadFailureText(fins_temp)) blockReadFailed = true; // ★R11
                                                shuju_temp += GetMiddleValue(fins_temp, " ", "\r");
                                            }
                                            else if (par.Value[4] == "long" && j % 2 == 0)
                                            {
                                                xuanzhong_temp = blkQishi - baseAddr + j;
                                                // 读取字符串
                                                DemoUtils.ReadResultRender1(ReadLocked(() => busTcpClient.ReadInt32((blkQishi + j).ToString())), (blkQishi + j).ToString(), out fins_temp);
                                                CommGridHelper.SetPollCell(_gridUi, fins_data, xuanzhong_temp, fins_temp);
                                                if (CommTriggerHelper.IsReadFailureText(fins_temp)) blockReadFailed = true; // ★R11
                                                shuju_temp += GetMiddleValue(fins_temp, " ", "\r");
                                            }
                                            else if (par.Value[4] == "float" && j % 2 == 0)
                                            {
                                                xuanzhong_temp = blkQishi - baseAddr + j;
                                                // 读取字符串
                                                DemoUtils.ReadResultRender1(ReadLocked(() => busTcpClient.ReadFloat((blkQishi + j).ToString())), (blkQishi + j).ToString(), out fins_temp);
                                                CommGridHelper.SetPollCell(_gridUi, fins_data, xuanzhong_temp, fins_temp);
                                                if (CommTriggerHelper.IsReadFailureText(fins_temp)) blockReadFailed = true; // ★R11
                                                shuju_temp += GetMiddleValue(fins_temp, " ", "\r");
                                            }
                                                        ...
                                                        if (CommTriggerHelper.CheckTriggerCondition(dataVal, mode, tv1, tv2))
                                                        {
                                                            if (!_triggerLatch.TryGetValue(pap.Key, out bool latched) || !latched)
                                                            {
                                                                ...
                                                                try
                                                                {
                                                                    ...
                                                                    if (hTrig != null)
                                                                    {
                                                                        hTrig(this, new SelectionChangedEventArgs(dataVal, pap.Key.ToString(), pap.Key.ToString(), _modbusLink.LinkId));
                                                                        trigOk = true;
                                                                    }
                                                                }
                    ...
                    try
                    {
                        ...
                        foreach (var par in finsSnapshot)
                        {
                            ...
                            if (fmt == "int")
                            {
                                ...
                                {
                                    // ★M9 修复：原降级逐一写不检查结果、随后伪造 CreateSuccessResult 照常清槽；
                                    //   现任一逐写失败即整批失败（不伪造成功）；H3：失败不清槽，下次自动重试。
                                    string aggErr = "批量写失败(" + batchResult.Message + ")，降级逐地址写：";
                                    for (int j = 0; j < vals.Length; j++)
                                    {
                                        OperateResult r = busTcpClient.Write((addr_start + j).ToString(), vals[j]);
                                        if (!r.IsSuccess) { writeAllOk = false; aggErr += " |" + (addr_start + j) + ":" + r.Message; }
                                    }
                                    fins_temp = aggErr;
                                }
                                else
                                {
                                    writeAllOk = DemoUtils.WriteResultRender1(() => batchResult, addr_start.ToString(), out fins_temp);
                                }
                                for (int j = 0; j < vals.Length; j++)
                                {
                            ...
                            else if (fmt == "long")
                            {
                                ...
                                writeAllOk = DemoUtils.WriteResultRender1(() => busTcpClient.Write(addr_start.ToString(), vals), addr_start.ToString(), out fins_temp);
                            ...
                            else if (fmt == "string")
                            {
                                ...
                                {
                                    int xuanzhong_temp = addr_start - int.Parse(address_qishi.ToString()) + j;
                                    if (!DemoUtils.WriteResultRender1(() => busTcpClient.Write((addr_start + j).ToString(), parts[j].Trim()), (addr_start + j).ToString(), out fins_temp))
                                        writeAllOk = false;
                                    CommGridHelper.SetPollCell(_gridUi, fins_data, xuanzhong_temp, fins_temp);
                                }
                            }
        ...
        private void comboBox29_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (chushihua)
            {
                ...
                WarnSameFeedbackBlock(13, comboBox29.Text);   // ★N4：心跳与相机反馈同块告警
            }
        }
