...
{
    /// <summary>
    /// FINS 多连接配置读写（阶段 3-A：只读写 ini，不接管任何运行逻辑）。
    ///
    /// 把 test.ini 中 N 条 FINS 连接的全部段加载/保存成 FinsLinkConfig 列表。
    /// 命名规则（与总纲第 4 节一致，连接 1 空后缀零迁移）：
    ///
    ///   [fins]        / [finsN]        连接参数（ip/port/cell/local/abcd/轮询...）
    ///   [N]           / [N_finsN]      数据块（name/qishi/changdu/gaodiwei/geshi）——连接1为纯数字段
    ///   [cN]          / [cN_finsN]     相机绑定（chufa/fanhuizhi/fanhuien/fankui/chukufangshi/chufazhi1/2）
    ///   [change]      / [change_finsN] 切型字符（键 1..12）——连接1为 [change]（与无协议共用同名段）
    ///   [path]        / [path_finsN]   方案路径（键 1..12）——连接1为 [path]（与无协议共用同名段）
    /// </summary>
    public static class FinsIniStore
    {
        ...
        internal static string ConnSection(int link) => link == 1 ? "fins" : "fins" + link;
        // 连接 1 数据块段为纯数字 [N]；连接 2+ 为 [N_finsN]
        internal static string BlockSection(int link, int n) => link == 1 ? n.ToString() : n + "_fins" + link;
        internal static string CameraSection(int link, int n) => "c" + n + (link == 1 ? "" : "_fins" + link);
        // 连接 1 切型/路径段为 [change]/[path]（与无协议共用）；连接 2+ 加后缀
        internal static string ChangeSection(int link) => link == 1 ? "change" : "change_fins" + link;
        internal static string PathSection(int link) => link == 1 ? "path" : "path_fins" + link;

        /// <summary>加载全部连接（连接 1 始终存在；连接 2..MaxLinks 仅当对应段非空时才纳入）。</summary>
        public static List<FinsLinkConfig> LoadAll(ClassIni ini)
        {
            if (ini == null || string.IsNullOrEmpty(ini.FileName))
                return new List<FinsLinkConfig>();

            var list = new List<FinsLinkConfig>();
            list.Add(Load(ini, 1));
        ...
        public static FinsLinkConfig Load(ClassIni ini, int link)
        {
            string sec = ConnSection(link);
            var cfg = new FinsLinkConfig
            {
                LinkId = link,
                Name = ini.ReadString(sec, "name", "FINS-" + link),
                ...
                Local = ini.ReadString(sec, "local", "192").Replace("\0", ""),
                ...
                FinsEn = ini.ReadBool(sec, "fins_en", false),
                FinsLunxunen = ini.ReadBool(sec, "fins_lunxunen", false),
        ...
        public static void SaveAll(ClassIni ini, List<FinsLinkConfig> configs)
        {
        ...
        public static void Save(ClassIni ini, FinsLinkConfig cfg)
        {
            ...
            ini.WriteString(sec, "local", cfg.Local);
            ...
            ini.WriteBool(sec, "fins_en", cfg.FinsEn);
            ini.WriteBool(sec, "fins_lunxunen", cfg.FinsLunxunen);
