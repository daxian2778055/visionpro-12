...
namespace WindowsFormsApplication1
{
    ...
    public class FinsRuntime : ICommLink
    {
        ...
        ///   读、写、重连三处同锁。RTU/TCP 上下文的回写仍委托宿主连接 1 的客户端，直接复用宿主 _ioSync
                                ...
                                foreach (var par in ctx.FinsBlocks)
                                {
                                    ...
                                    for (int j = 0; j < int.Parse(par.Value[2]); j++)
                                    {
                                        if (par.Value[4] == "int")
                                        {
                                            ...
                                            DemoUtils.ReadResultRender1(_link.Client.ReadInt16("D" + (baseAddr + j).ToString()), "D" + (baseAddr + j).ToString(), out fins_temp);
                                        ...
                                        {
                                            xuanzhong_temp = baseAddr - ctx.AddressBase + j;
                                            DemoUtils.ReadResultRender1(_link.Client.ReadString("D" + (baseAddr + j).ToString(), 1), "D" + (baseAddr + j).ToString(), out fins_temp);
                                            ctx.UpdatePollCell(xuanzhong_temp, fins_temp);
                                            if (CommTriggerHelper.IsReadFailureText(fins_temp)) blockReadFailed = true; // ★R11
                                            shuju_temp += ctx.MiddleValue(fins_temp, " ", "\r");
                                        }
                                        ...
                                        {
                                            xuanzhong_temp = baseAddr - ctx.AddressBase + j;
                                            DemoUtils.ReadResultRender1(_link.Client.ReadInt32("D" + (baseAddr + j).ToString()), "D" + (baseAddr + j).ToString(), out fins_temp);
                                            ctx.UpdatePollCell(xuanzhong_temp, fins_temp);
                                            if (CommTriggerHelper.IsReadFailureText(fins_temp)) blockReadFailed = true; // ★R11
                                            shuju_temp += ctx.MiddleValue(fins_temp, " ", "\r");
                                        }
                                        ...
                                        {
                                            xuanzhong_temp = baseAddr - ctx.AddressBase + j;
                                            DemoUtils.ReadResultRender1(_link.Client.ReadFloat("D" + (baseAddr + j).ToString()), "D" + (baseAddr + j).ToString(), out fins_temp);
                                            ctx.UpdatePollCell(xuanzhong_temp, fins_temp);
                                            if (CommTriggerHelper.IsReadFailureText(fins_temp)) blockReadFailed = true; // ★R11
                                            shuju_temp += ctx.MiddleValue(fins_temp, " ", "\r");
                                        }
