...
{
    /// <summary>
    /// Modbus-TCP 单条连接（2~4）的配置窗体（阶段 6：让“连接设备”管理器像 FINS 一样增删多连接）。
    ///
    /// 全部控件在 FormModbusTcpLinkConfig.Designer.cs 声明，可直接在 VS 设计器编辑。
    /// 数据用 DataTable 绑定三个 DataGridView（数据块 / 相机绑定 / 切型方案），
    /// 保存时经 <see cref="ModbusTcpIniStore"/> 写回本连接的 [modbustcpN] 段族，
    /// 并触发 <see cref="Saved"/> 让管理器把运行时应用到 FormModbus 单例
    /// （写 ini 并即时启停对应连接的后台运行时）。
    ///
    /// 连接 1 仍走原 FormModbus 单例窗口，不在此窗体编辑（避免重复配置源）。
    /// </summary>
    public partial class FormModbusTcpLinkConfig : Form
    {
        ...
        public FormModbusTcpLinkConfig()
        {
            InitializeComponent();
            SetupGrids();
        }
        ...
        {
            if (_ini == null) return;
            var cfg = ModbusTcpIniStore.Load(_ini, _linkId);
            ...
            for (int n = 1; n <= ModbusTcpIniStore.MaxChangeTypes; n++)
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                var cfg = new ModbusTcpLinkConfig { LinkId = _linkId };
                ...
                foreach (DataRow r in _dtCameras.Rows)
                {
                    ...
                    if (camNo < 1 || camNo > ModbusTcpIniStore.MaxCameras) continue;
                ...
                {
                    int no = ToInt(r, "No", 0);
                    if (no < 1 || no > ModbusTcpIniStore.MaxChangeTypes) continue;
                    cfg.ChangeTypes[no] = Str(r, "Change");
                    cfg.SchemePaths[no] = Str(r, "Path");
                }

                ModbusTcpIniStore.Save(_ini, cfg);
                lblInfo.Text = "已保存并应用：Modbus-TCP 连接 " + _linkId;
                Saved?.Invoke(_linkId);
            }
            catch (Exception ex)
            {
                lblInfo.Text = "保存失败：" + ex.Message;
                MessageBox.Show(this, "保存失败：" + ex.Message, "Modbus-TCP 连接配置", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
